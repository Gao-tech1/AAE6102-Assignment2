...
% Updated on 2025/03/11 with multicorrelator.
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        for loopCnt =  1:codePeriods
            ...
            % Multiple correlators for different offsets using
            % interpolation for AAE6102 Assignment1
            offsets = linspace(-0.4, 0.4, 9); % Generate 9 offsets from -0.4 to 0.4
            multiCodes = cell(length(offsets),1);
            
            for i = 1:length(offsets)
                % Generate the code phase for the current offset
                tcode = (remCodePhase + offsets(i)) : codePhaseStep : ...
                    ((blksize-1)*codePhaseStep + remCodePhase + offsets(i));
                % Interpolate the code phase to the nearest integer index
                tcode2 = ceil(tcode) + 1;
                multiCodes{i} = caCode(tcode2);
            end
            ...
            % Calculate multiple correlator values
            I_multi = [I_E];
            Q_multi = [Q_E];
            for i = 1:length(offsets)
                I_multi = [I_multi, sum(multiCodes{i} .* iBasebandSignal)];
                Q_multi = [Q_multi, sum(multiCodes{i} .* qBasebandSignal)];
            end
            I_multi = [I_multi, I_L];
            Q_multi = [Q_multi, Q_L];
            trackResults(channelNr).I_multi{loopCnt} = I_multi;
            trackResults(channelNr).Q_multi{loopCnt} = Q_multi;
        ...
        % generateCorrelationPlots(trackResults(channelNr),settings);

        % Analyze urban interference impact
        % plotpeakvalley(trackResults(channelNr));
